...
SOP_class::SOP_class(std::string SOP_class_UID,
                     std::map<DIMSE_SERVICE_GROUP,
                        std::function<response(dimse_pm*, data::dataset::commandset_data cdata, std::unique_ptr<iod>)> > handler):
   sop_uid {SOP_class_UID}, operations {handler}
{
}

response SOP_class::operator()(dimse_pm* pm, DIMSE_SERVICE_GROUP op, data::dataset::commandset_data cdata, std::unique_ptr<iod> data) const
{
   return operations.at(op)(pm, std::move(cdata), std::move(data));
}
